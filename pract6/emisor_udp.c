...
#define PUERTO 4950 /* Puerto en el que escucha el receptor */
...
int main(int argc, char ** argv) {
  ...
  num_bytes = sendto(
    ...
    strlen(argv[2]),
    0,
    (struct sockaddr *) &receptor_addr,
    sizeof(receptor_addr)
    );
  if (num_bytes < 0) {
    perror("sendto");
    exit(1);
  ...
  printf(
    "Enviados %d bytes a %s\n",
    num_bytes,
    inet_ntoa(receptor_addr.sin_addr)
    );
